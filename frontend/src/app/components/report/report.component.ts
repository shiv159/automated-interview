...
export class ReportComponent implements OnInit {
  ...
  readonly busy = signal(false); readonly message = signal(''); readonly report = signal<Report | null>(null);
  async ngOnInit() { if (!this.id) { this.router.navigate(['/']); return; } this.busy.set(true); try { this.report.set(await this.sessionService.getReport(this.id)); } catch (e: any) { this.message.set(e.message || 'Report unavailable.'); } finally { this.busy.set(false); } }
  ...
  async deleteSession() { if (!this.id) return; this.busy.set(true); try { await this.sessionService.deleteSession(this.id); this.report.set(null); this.router.navigate(['/']); } catch (e: any) { this.message.set(e.message || 'Failed to delete session.'); } finally { this.busy.set(false); } }
}
