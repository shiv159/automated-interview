...
export class CandidateReviewComponent implements OnInit {
  ...
  jobFile: File | null = null; resumeFile: File | null = null; yearsExperience = 3; syntheticDataAttested = false; dragTarget = ''; previewKind = ''; previewText = ''; skillFilter = 'ALL';
  ...
  loadDemo() { this.jobFile = new File(['Role: Java / Spring engineer\nSkills: Java, Spring Boot, SQL'], 'demo-java-spring-role.txt', { type: 'text/plain' }); this.resumeFile = new File(['Candidate: Demo Applicant\nExperience: 4 years Java and Spring'], 'demo-resume.txt', { type: 'text/plain' }); this.syntheticDataAttested = true; this.message.set('Demo materials loaded — ready to review.'); }
  setFile(kind: 'job' | 'resume', event: Event) { const file = (event.target as HTMLInputElement).files?.[0] ?? null; if (kind === 'job') this.jobFile = file; else this.resumeFile = file; }
  onDrop(kind: 'job' | 'resume', event: DragEvent) { event.preventDefault(); this.dragTarget = ''; const file = event.dataTransfer?.files?.[0] ?? null; if (kind === 'job') this.jobFile = file; else this.resumeFile = file; }
  removeFile(kind: 'job' | 'resume') { if (kind === 'job') this.jobFile = null; else this.resumeFile = null; if (this.previewKind === kind) this.previewKind = ''; }
  async previewFile(kind: 'job' | 'resume') { const file = kind === 'job' ? this.jobFile : this.resumeFile; if (!file) return; this.previewKind = kind; this.previewText = file.type.includes('text') || file.name.endsWith('.txt') ? (await file.text()).slice(0, 900) : 'Preview is available after the document is extracted by the analysis service.'; }
  fileSize(file: File): string { return file.size < 1024 * 1024 ? `${Math.max(1, Math.round(file.size / 1024))} KB` : `${(file.size / 1024 / 1024).toFixed(1)} MB`; }
  get filteredClaims(): any[] { const session = this.result(); if (!session) return []; return session.jobSkills.filter((claim: any) => this.skillFilter === 'ALL' || (this.skillFilter === 'MATCHED' ? session.matchedSkills.includes(claim.skillId) : session.missingSkills.includes(claim.skillId))); }
