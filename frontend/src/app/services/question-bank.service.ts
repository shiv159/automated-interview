...
export interface QuestionSummary {
  ...
  type: 'TECHNICAL' | 'BEHAVIORAL'; primarySkill: string | null; secondarySkills: string; difficulty: string | null;
...
export interface AnalysisQuestion { stem: string; type: 'TECHNICAL' | 'BEHAVIORAL' | null; primarySkill: string | null; secondarySkills: string[]; difficulty: string | null; tags: string[]; idealAnswer: string | null; status: string; errorCode: string | null; }
...
export class QuestionBankService {
  ...
  async importQuestions(file: File): Promise<{ createdCount: number; updatedCount: number; skippedCount: number; questions: QuestionSummary[]; errors: unknown[] }> {
    const body = new FormData();
    body.append('questionsFile', file);
    return firstValueFrom(this.http.post<{ createdCount: number; updatedCount: number; skippedCount: number; questions: QuestionSummary[]; errors: unknown[] }>('/api/v1/question-bank/import', body));
  }

  async analyzeQuestions(file: File): Promise<QuestionAnalysis> {
    const body = new FormData(); body.append('questionsFile', file);
    return firstValueFrom(this.http.post<QuestionAnalysis>('/api/v1/question-bank/analyze', body));
  }

  async importDraft(payload: { questions: AnalysisQuestion[]; approvedSkills: SkillSuggestion[] }): Promise<any> {
    return firstValueFrom(this.http.post('/api/v1/question-bank/import-draft', {
      questions: payload.questions.map(({ stem, type, primarySkill, secondarySkills, difficulty, tags, idealAnswer, status }) => ({ stem, type, primarySkill, secondarySkills, difficulty, tags, idealAnswer, status })),
      approvedSkills: payload.approvedSkills.map(({ id, displayName, aliases }) => ({ id, displayName, aliases }))
    }));
  }
