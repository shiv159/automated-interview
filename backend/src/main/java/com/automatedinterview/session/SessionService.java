...
public class SessionService {
    ...
    public CreatedSession create(MultipartFile jobFile, MultipartFile resumeFile, int yearsExperience, boolean attested, String roleTitle) {
        ...
        try {
            ...
            jdbc.sql("""
                INSERT INTO interview_session (id, token_hash, state, years_experience, difficulty, profile_match, expires_at, role_title, unsupported_requirements, soft_skill_requirements, domain_requirements)
                VALUES (:id, :tokenHash, 'READY', :years, :difficulty, :profileMatch, :expiresAt, :roleTitle, CAST(:unsupported AS jsonb), CAST(:soft AS jsonb), CAST(:domain AS jsonb))
                ...
                .param("expiresAt", java.sql.Timestamp.from(persistedExpiresAt)).param("roleTitle", normalizedRoleTitle).param("unsupported", json(jobAnalysis.unsupportedTechnicalSkills())).param("soft", json(jobAnalysis.softSkillRequirements())).param("domain", json(jobAnalysis.domainRequirements())).update();
            saveClaims(id, "JOB", jobClaims, resumeSkills);
            saveClaims(id, "RESUME", resumeClaims, jobSkills);
            return new CreatedSession(new SessionResponse(id, persistedExpiresAt, normalizedRoleTitle, difficulty, profileMatch, markMatches(jobClaims, resumeSkills), markMatches(resumeClaims, jobSkills), matched, missing, jobAnalysis.unsupportedTechnicalSkills(), jobAnalysis.softSkillRequirements(), jobAnalysis.domainRequirements()), token);
    ...
    public SessionResponse snapshot(UUID id, String token) {
        ...
        SessionRow row = jdbc.sql("SELECT id, state, difficulty, profile_match, expires_at, COALESCE(role_title, '') AS role_title, unsupported_requirements, soft_skill_requirements, domain_requirements FROM interview_session WHERE id = :id AND token_hash = :tokenHash")
            .param("id", id).param("tokenHash", hash(token)).query((rs, n) -> new SessionRow(
                rs.getObject("id", UUID.class), rs.getString("difficulty"), rs.getDouble("profile_match"), rs.getTimestamp("expires_at").toInstant(), rs.getString("role_title"), rs.getString("unsupported_requirements"), rs.getString("soft_skill_requirements"), rs.getString("domain_requirements"))).optional()
